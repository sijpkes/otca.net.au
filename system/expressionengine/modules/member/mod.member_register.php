...
class Member_register extends Member {
	...
	public function registration_form() {
		...
		// add OTCA institution check
		$type =  ee() -> input -> get("T");
		$hash =  ee() -> input -> get("C");

		$hash_type = $type . "_uri_hash";

		$result =  ee() -> db -> query("SELECT id, name, type, expiry_date FROM otca_institutions WHERE $hash_type = '$hash' LIMIT 1");

		$registration_form = $type . "_registration_form";
		...
		if ($query -> num_rows() == 0) {
			...
			foreach ($query->result_array() as $row) {
				...
				if (($type == "educator" && $row["m_field_name"] !== "student_number") || $type === "student") {
                    ...
                    $temp = str_replace("{institution_name}", $institution_name, $temp);
		...
		// Parse languge lines
		$reg_form = $this -> _var_swap($reg_form, array('lang:username_length' => $un_min_len, 'lang:password_length' => $pw_min_len, 'form:localization' =>  ee() -> localize -> timezone_menu(), 'form:time_format' => $tf, 'form:language' =>  ee() -> functions -> language_pack_names('english')));
		...
		$header_pfx = ucwords($type);
		$header = "<h1>$header_pfx Registration</h1><h2 style='margin-top: -10px'>$institution_name</h2>";
	...
	public function register_member() {
		...
		$VAL = new EE_Validate( array('member_id' => '', 'val_type' => 'new', // new or update
		'fetch_lang' => TRUE, 'require_cpw' => FALSE, 'enable_log' => FALSE, 'username' => trim_nbs($_POST['username']), 'cur_username' => '', 'screen_name' => trim_nbs($_POST['screen_name']), 'cur_screen_name' => '', 'password' => $_POST['password'], 'password_confirm' => $_POST['password_confirm'], 'cur_password' => '', 'email' => trim($_POST['email']), 'cur_email' => ''));
	...
	private function _do_form_query() {
		if (ee() -> input -> get_post('board_id') !== FALSE && is_numeric(ee() -> input -> get_post('board_id'))) {
			return  ee() -> db -> select('board_forum_url, board_id, board_label') -> where('board_id', (int) ee() -> input -> get_post('board_id')) -> get('forum_boards');
		}

		return  ee() -> db -> select('board_forum_url, board_id, board_label') -> where('board_id', 1) -> get('forum_boards');
	}
	...
	public function activate_member() {
		...
		if ($id == FALSE) {
			...
			$data = array('title' => lang('mbr_activation'), 'heading' => lang('error'), 'content' => lang('invalid_url'), 'link' => array($return, $site_name));
		...
		if ($query -> num_rows() == 0) {
			$data = array('title' => lang('mbr_activation'), 'heading' => lang('error'), 'content' => lang('mbr_problem_activating'), 'link' => array($return, $site_name));
		...
		$data = array('title' => lang('mbr_activation'), 'heading' => lang('thank_you'), 'content' => lang('mbr_activation_success') . "\n\n" . lang('mbr_may_now_log_in'), 'link' => array($return, $site_name));
