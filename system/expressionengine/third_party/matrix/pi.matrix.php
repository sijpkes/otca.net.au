...
if($group_id != 1 && $group_id != 6 && $group_id != 7) return "<h1>Restricted Access.</h1>";

$id = ee()->TMPL->fetch_param('id');
$this->student_id = ee()->TMPL->fetch_param('member_id');

$sql = "SELECT screen_name, email FROM exp_members WHERE member_id = '$this->student_id' LIMIT 1";
...
AND title.entry_id = ev.entry_id AND student.member_id = title.author_id AND student.member_id = '$this->student_id' AND data.entry_id = '$this->id'
...
$sql = "SELECT title.entry_date, data.entry_id, title.author_id, title.title, data.field_id_6 as self_assessment, av.matrix_ids as supervisor_assessment,
	...
	AND student.member_id ='$this->student_id' ORDER BY m.group_id, av.date_assessed DESC, data.entry_id";
...
$form .= $this->educatorHereDocJS($this->id, $this->member_id, $student_screen_name, $student_email, $assessed_items_js, $self_assessed_item_js);
return $form;
}
...
private static function educatorHereDocJS($id, $student_id, $student_screen_name, $student_email, $assessed_items_js, $self_assessed_item_js) {
    $js = <<<EOT
    (function( \$ ) {
