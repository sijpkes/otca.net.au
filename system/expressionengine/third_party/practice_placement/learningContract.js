...
$.fn.learningContract = function(params) {
	...
	/*$.fn.addIDs = function() {
		...
		$(tableo).find('tr').each(function(rowi, rowo) {
		    var isBAP = $(rowo).closest('table').hasClass('bap');  // exception for being a prof
			if(rowi > 0) { // skip row headers
				$(rowo).find('td').each(function(coli, colo) {
					if(!isBAP && coli > 0) { // skip column headers except for BAP
...
$("<?= $jquery_selector ?>").learningContract({planning: <?=$planning?>, evidence: <?=$evidence?>, objectives: <?=$objectives?>});
