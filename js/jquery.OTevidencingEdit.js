...
$sql = "SELECT data.entry_id, data.field_id_6 as self_assessment, av.matrix_ids as supervisor_assessment, av.date_assessed FROM  exp_channel_data data LEFT JOIN (exp_channel_titles title , otca_evidence ev, otca_evidence_validated av) ON (data.entry_id = ev.entry_id AND title.entry_id = ev.entry_id AND ev.entry_id = av.evidence_id) WHERE ev.entry_id = '$id'";
...
if ($query->num_rows() > 0)
{
    ...
    {
		$assess_array[] = array(date_assessed => $row['date_assessed'], supervisor_assessment => $row['supervisor_assessment']);
	}
}

$assessed_items_js = "var assessed_items = ".json_encode($assess_array).";"
?>
(function( $ ) {
