...
class Member_model extends CI_Model {
	 ...
	 * Get Members
	 *
	 * Get a collection of members, modified for OTCA implementation.
	 * This gets members by institution selected by the admin member_id viewing them.
	 *
	 * @access	public
	 * @param	int
	 * @param	int
	...
	function get_members($group_id = '', $limit = '', $offset = '', $search_value = '', $order = array(), $column = 'all') {
		...
		$this -> db -> select("members.username, members.member_id, members.screen_name, members.email, members.join_date, 
									members.last_visit, members.group_id, members.member_id, members.in_authorlist,
									member_data.m_field_id_1 as first_name, member_data.m_field_id_2 as last_name,
		...
		if (is_array($order) && count($order) > 0) {
			foreach ($order as $key => $val) {
				/* OTCA patch 25-7-13 */
				if ($key !== 'nominated') {// exempt OTCA nominated field
	...
	/*
	 *	OTCA institution member count
	 *
	 */
	function get_institution_member_count($group_id = FALSE, $institution_id = 0) {

		if ($institution_id === 0) {
			return get_member_count($group_id);
		}
		...
		if ($group_id != '') {
			$this -> db -> select('member_id');
			$this -> db -> join('otca_member_fields', 'members.member_id = otca_member_fields.member_id');
			$this -> db -> where('otca_member_fields.group_id', $group_id);
			$this -> db -> where('otca_member_fields.institution_id', $institution_id);
			$query = $this -> db -> get('members');
		...
		$this -> db -> from("member_groups");
		$this -> db -> from("members");
		$this -> db -> from("otca_member_fields");
		$this -> db -> where("members.member_id = " . $this -> db -> dbprefix("otca_member_fields.member_id"));
		$this -> db -> where("otca_member_fields.group_id = " . $this -> db -> dbprefix("member_groups.group_id"));
		$this -> db -> where("members.group_id = " . $this -> db -> dbprefix("member_groups.group_id"));
		$this -> db -> where("member_groups.site_id", $this -> config -> item('site_id'));
		$this -> db -> where("otca_member_fields.institution_id", $institution_id);
	...
	public function update_member_entry_stats($member_ids = array()) {
		...
		foreach ($member_ids as $member_id) {
			...
			// Update member stats
			$this -> db -> where('member_id', $member_id);
			$this -> db -> update('members', array('total_entries' => $new_stats['count'], 'last_entry_date' => $new_stats['entry_date']));
		}
	}
	...
	function get_authors($author_id = FALSE, $limit = FALSE, $offset = FALSE) {
		...
		$groups = $this -> db -> select('group_id') -> where('include_in_authorlist', 'y') -> where('site_id', $this -> config -> item('site_id')) -> get('member_groups') -> result_array();
	...
	function get_group_layout($member_group = '', $channel_id = '') {
		...
		return $this -> layout_model -> get_layout_settings(array('site_id' => $this -> config -> item('site_id'), 'channel_id' => $channel_id, 'member_group' => $member_group));
	}
